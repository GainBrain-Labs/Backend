...
from app.exceptions.repository_errors import NotFoundError, FoundError
    ...
    async def create_user(self,user_data: UserCreate):
        try:
            try:
                ...
                if user:
                    raise FoundError(f"user already exists with email {user_data.email}")
            ...
            try:
                ...
                if user:
                    raise FoundError(f"user already exists with username {user_data.username}")
            except NotFoundError as e:
